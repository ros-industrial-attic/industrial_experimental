...
"""
base_package_generator

Base class interface to a package generator
"""
class base_package_generator():
...
"""
class support_package_generator(base_package_generator):
  ...
  def generate_package(self, prefix, model, num_joints, author, author_email, version, template_path):
    ...
    em_params = self.__load_em_params__(package, model, num_joints, author, author_email, version)
  ...
  def __load_em_params__(self, package, model, num_joints, author, author_email, version):
...
"""
moveit_package_generator

This class encapsulates functionality to generate standard ROS-I MoveIt 
packages.  This class uses templates (em.py) to generate package files.
Templates can be changed to alter the generated files.
"""
class moveit_package_generator(base_package_generator):
  ...
  def generate_package(self, prefix, model, num_joints, assistant, template_path):
    ...
    #Load arguments into em params
    em_params = self.__load_em_params__(moveit_package, support_package, model, num_joints)
    
    #Start setup assistant
    if assistant:
      self.__setup_assistant__()
    
    #Create package directory structure (if it doesn't exist)
    mkdir(moveit_package)
    file_path = moveit_package + "/launch"
    mkdir(file_path)
    self.__generate_controller_manager_launch__(em_params, prefix, template_path, file_path, model)
    self.__generate_planning_execution_launch__(em_params, template_path, file_path)
    
    file_path = moveit_package + "/config"
    mkdir(file_path)
    self.__generate_controllers_yaml__(em_params, template_path, file_path)
    
    print("TODO: CHECK JOINT LIMITS YAML FOR ACCEL/DECEL LIMITS")
        
    
  def __load_em_params__(self, moveit_package, support_package, model, num_joints):
  ...
  def __generate_controller_manager_launch__(self, em_params, prefix, template_path, file_path, model):
    if prefix:
      file_name = prefix + "_" + model  + "_moveit_controller_manager.launch.xml"
    else:
      file_name = model  + "_moveit_controller_manager.launch.xml"
    self.__generate_empy_file__(em_params, template_path, "controller_manager.empy", file_path, file_name)
    
  def __generate_planning_execution_launch__(self, em_params, template_path, file_path):
    self.__generate_empy_file__(em_params, template_path, "moveit_planning_execution.empy", file_path, "moveit_planning_execution.launch")
    
  
  def __generate_controllers_yaml__(self, em_params, template_path, file_path):
    self.__generate_empy_file__(em_params, template_path, "controllers.empy", file_path, "controller.yaml")
    
  def __setup_assistant__(self):
